...
void writeExpressionsToFile() {
	...
	if (!inputFile) {
		printf("Îøèáêà îòêðûòèÿ ôàéëà input.txt\n");
		return;
	}

	char expression[256];
	printf("Ââåäèòå âûðàæåíèÿ (äëÿ çàâåðøåíèÿ ââåäèòå 'exit'):\n");
	...
	fclose(inputFile); 
	printf("Âûðàæåíèÿ çàïèñàíû â ôàéë input.txt\n");
}
